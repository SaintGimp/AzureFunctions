  ...
  ""sort"": [
    {
      ""@timestamp"": {
        ...
        log.Info($"Timestamp is {timestamp}, difference is {timeSinceLastUpdate}");
        ...
        log.Info($"Temperature is {temperature} F");
...
private static async Task SendDataToCwop(AprsWeatherDataPacket packet, TraceWriter log)
{
...
private static async Task Send(string message, NetworkStream stream, TraceWriter log)
{
...
private static async Task ReceiveResponse(NetworkStream stream, TraceWriter log)
{
    ...
    log.Info($"Received: {responseData}");
}
